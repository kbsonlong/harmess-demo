...
### 严格工作流（不可跳过）：
1. **任务规划**：调用 `write_todos` 列出 A-D 的详细任务。
2. **任务指派（核心）**：
...
4. **最终交付**：只有当所有 TODO 标记为 `completed` 后，才允许输出最终报告。
...
def run_inspection(thread_id: str):
    ...
    agent = create_deep_agent(
        ...
        permissions=[
            ...
            FilesystemPermission(operations=["read", "write"], paths=[os.path.join(project_dir, "/**")], mode="deny")
        ],
    ...
    initial_state = {
        "messages": [
            HumanMessage(content=(
                ...
                "2) 在沙箱内执行 `python -m sandbox_inspector.cli run --max-findings 50` 获取巡检 JSON。\n"
                "3) 解析 JSON 文件，提取异常摘要。\n"
                "4) 根据异常摘要，指派 `infra_expert` 或 `workload_expert` 执行详细诊断。\n"
                "5) 等待所有专家完成诊断。\n"
                "6) 生成最终 Markdown 报告。\n"
                "请注意：规划完任务后，请立即开始指派 subagent 执行第一项任务。不要停下。"
            ))
        ]
    }
