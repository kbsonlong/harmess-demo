...
- 已具备：VictoriaLogs 查询工具（本机直连/代理访问，支持 VICTORIALOGS_BASE_URL/VICTORIALOGS_PROXY_URL）并接入运行时；修复 victorialogs_query 缺失 docstring 导致 Tool 创建失败；main.py 自动注入最新 release_failure 上下文；报告模板增加 时间线/证据索引/回滚点 章节要求
- 已补充：GitOps demo 手册 `docs/gitops_demo_manual.md`（手动触发 inject-failure/wait-fail、人工验证、Agent 归因分析）
